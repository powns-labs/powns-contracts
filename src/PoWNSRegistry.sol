...
/**
 * @title PoWNSRegistry
 * @notice Core registry for PoW Name Service
 * @dev Domains are ERC-721 NFTs, scarcity determined by PoW
 */
    ...
    ReentrancyGuard 
{
    ...
    ) ERC721("PoWNS", "POWNS") Ownable(msg.sender) {
    ...
    function renew(
        ...
        require(
            ...
            (domain.state == DomainState.GracePeriod && domain.owner == msg.sender),
            "Not authorized"
        );
        ...
        // Verify PoW
        (bool valid, ) = verifier.verify(name, domain.owner, msg.sender, nonce, target);
        ...
        // Extend expiration
        uint256 baseTime = domain.expires > block.timestamp ? domain.expires : block.timestamp;
        ...
        emit DomainRenewed(name, name, domain.owner, domain.expires, additionalYears);
    }
    ...
    function _isAvailable(bytes32 nameHash) internal view returns (bool) {
        ...
        return state == DomainState.Available || 
               state == DomainState.Released || 
               state == DomainState.Auction;
    }
    
    function _getCurrentState(bytes32 nameHash) internal view returns (DomainState) {
        Domain storage domain = domains[nameHash];
